...
public class Shell {
   ...
   * @param screenName = name of the screen in the project
   * @param blocklyJavascript = javascript to include
   * @param componentJSON = "wrapped" component JSON
   * @param isLiveWebAppBuild = is this a LiveWebApp build
   * @param isQrCodeBuild = is this a QR Code build
  ...
  public static StitchResult stitchBuildHTML (String projectName,
    ...
    // Including Bootstrap Library - No leading forward slash when dowmloading a zipped project
    if (isLiveWebAppBuild || isQrCodeBuild) {
      htmlStringBuilder.append("<link rel=\"stylesheet\" type=\"text/css\" href=\"/bootstrap/bootstrap.min.css\">\n");
    }
    else {
      htmlStringBuilder.append("<link rel=\"stylesheet\" type=\"text/css\" href=\"bootstrap/bootstrap.min.css\">\n");
    }
  ...
  public static void main(String args[]) 
  {
    ...
    Boolean isQrCodeBuild = false;
    String assetSrcPrefix = "assets/";
    try {
      StitchResult testResult = Shell.stitchBuildHTML("test", "test", blocklyJS, componentJSON, isLiveWebAppBuild, isQrCodeBuild, assetSrcPrefix);
